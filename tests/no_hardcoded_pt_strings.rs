        ...
        for entry in fs::read_dir(dir).unwrap() {
            ...
            if path.is_dir() {
                ...
                if content.contains("coletando dados do sistema") {
                    errors.push(format!(
                        "{}: contains 'coletando dados do sistema'",
                        path.display()
                    ));
                }
                if content.contains("Monitor {name} conectado. Ativando modo Expandir") {
                    errors.push(format!(
                        "{}: contains 'Monitor {{name}} conectado. Ativando modo Expandir'",
                        path.display()
                    ));
                }
                if content.contains("Monitor {name} desconectado.") {
                    errors.push(format!(
                        "{}: contains 'Monitor {{name}} desconectado.'",
                        path.display()
                    ));
                }
                if content.contains("Layout de telas: modo {} aplicado.") {
                    errors.push(format!(
                        "{}: contains 'Layout de telas: modo {{}} aplicado.'",
                        path.display()
                    ));
                }
...
fn no_hardcoded_pt_strings_in_ui_and_backend() {
    ...
    if !errors.is_empty() {
        panic!(
            "Found hardcoded strings or direct language matches instead of Messages struct:\n{}",
            errors.join("\n")
        );
    }
}
